...
# 1. Selección de Lavado con opción inicial vacía
lavado_seleccionado = st.selectbox(
    "Selecciona el tipo de lavado:",
    ["Selecciona una opción...", "DELT", "OVRW"]
)

# Validamos si ya seleccionaste algo
if lavado_seleccionado == "Selecciona una opción...":
    st.warning("⚠️ Por favor, selecciona un tipo de lavado en el menú de arriba para mostrar la información.")
else:
    ...
    if st.checkbox("🔍 Ver Comparativa Completa (BMP vs Lavado)", value=True):
        ...
        c1, c2 = st.columns(2)
        with c1: st.image(f"{path}{img_prefix}_frente_bmp.jpg", use_container_width=True)
        with c2: st.image(f"{path}{img_prefix}_frente_lavado.jpg", use_container_width=True)
        
        st.write("---")
        st.subheader(f"📸 VISTA TRASERA - {lavado_seleccionado}")
        c3, c4 = st.columns(2)
        with c3: st.image(f"{path}{img_prefix}_trasera_bmp.jpg", use_container_width=True)
        with c4: st.image(f"{path}{img_prefix}_trasera_lavado.jpg", use_container_width=True)
