...
if seleccion != "-- Selecciona --":
    ...
    with tab4:
        if seleccion in formulas_maestras:
            ...
            st.success(f"📋 COMPARATIVA DE DISEÑO Y LAVADO: {seleccion}")
            ...
            # --- FILA 1: DISEÑOS BMP ---
            st.write("**💻 DISEÑOS ORIGINALES (BMP):**")
            col1, col2 = st.columns(2)
            with col1:
                img = buscar_img(f"{seleccion}_frente_bmp")
                if img: st.image(img, caption="BMP Frente", use_column_width=True)
                else: st.info(f"Subir: {seleccion}_frente_bmp.png")
            with col2:
                img = buscar_img(f"{seleccion}_trasera_bmp")
                if img: st.image(img, caption="BMP Trasera", use_column_width=True)
                else: st.info(f"Subir: {seleccion}_trasera_bmp.png")

            # --- FILA 2: PRENDAS LAVADAS ---
            st.write("**🧼 RESULTADO FINAL (LAVANDERÍA):**")
            col3, col4 = st.columns(2)
            with col3:
                img = buscar_img(f"{seleccion}_frente_lavado")
                if img: st.image(img, caption="Prenda Lavada - Frente", use_column_width=True)
                else: st.info(f"Subir: {seleccion}_frente_lavado.png")
            with col4:
                img = buscar_img(f"{seleccion}_trasera_lavado")
                if img: st.image(img, caption="Prenda Lavada - Trasera", use_column_width=True)
                else: st.info(f"Subir: {seleccion}_trasera_lavado.png")
