...
if seleccion != "-- Selecciona --":
    ...
    def mostrar_metas(pzas, intensidad=None):
        ...
        with c3: 
            if intensidad:
                st.metric("⚡ Intensidad Láser", intensidad)
            else:
                st.metric("⚡ Intensidad", "N/D")
        
        st.table(pd.DataFrame({"Turno": ["8h", "10h", "12h"], "Meta": [pzas*8, pzas*10, pzas*12]}))

    with tab1: 
        int_t = config.get("intensidades", {}).get("twin")
        mostrar_metas(pzas_hora_base, int_t)
        
    with tab2: 
        int_fm = config.get("intensidades", {}).get("flexi_m")
        mostrar_metas(pzas_hora_base + 8, int_fm)
        
    with tab3: 
        int_fmesa = config.get("intensidades", {}).get("flexi_mesa")
        mostrar_metas(pzas_hora_base - 5, int_fmesa)
